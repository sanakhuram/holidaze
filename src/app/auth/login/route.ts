...
export async function POST(req: Request) {
    try {
        ...
        const res = NextResponse.json({ data: { ok: true } }, { status: 200, headers: { Vary: "Cookie" } });
        ...
        if (err instanceof Error) {
            return NextResponse.json({ error: err.message }, { status: 400, headers: { Vary: "Cookie" } });
        }
        return NextResponse.json({ error: "Unexpected error" }, { status: 400, headers: { Vary: "Cookie" } });
    }
}
