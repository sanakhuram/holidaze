...
export default function BookingSection({ venue }: { venue: VenueWithExtras }) {
        ...
        try {
            await toast.promise(
                (async () => {
                    ...
                    if (!r.ok)
                        throw new Error(
                            data?.errors?.[0]?.message || data?.message || "Booking failed"
                        );
                })(),
                {
    ...
    return (
        <section className="rounded-sm border border-amber-500 p-5 shadow-sm md:p-6">
            <h2 className="mb-4 text-center text-xl font-semibold text-wine md:mb-6">
                Book Now
            </h2>
            ...
            <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
                <div className="rounded-sm border border-amber-500 p-5 shadow-sm">
                    ...
                    <h3 className="mb-2 text-center text-amber-600 font-semibold text-lg">
                        Amenities
                    </h3>
